...
* if you use xref:testing/testcontext-framework/support-classes.adoc#testcontext-junit-jupiter-extension[`SpringExtension`] + JUnit Jupiter -> supported annotations
...
* := composed annotation /
  ** == `@ExtendWith(SpringExtension.class)` + `@ContextConfiguration`
    ...
    *** `@ContextConfiguration` -- from -- Spring TestContext Framework
  ** uses
    *** | class level
      **** -- replacement for -- `@ContextConfiguration`
  ** 's configuration options vs `@ContextConfiguration` & `@SpringJUnitConfig` 's configuration options
    *** `value` attribute / allows declaring component classes
* _Example1:_ to specify a configuration class
...
* _Example2:_ to specify the location of a configuration file
...
* Check
  ...
  ** {spring-framework-api}/test/context/ContextConfiguration.html[`@ContextConfiguration`]
...
`@WebAppConfiguration`
    *** `@ExtendWith(SpringExtension.class)` -- from -- JUnit Jupiter
    *** `@ContextConfiguration` -- from -- Spring TestContext Framework
    *** `@WebAppConfiguration` -- from -- Spring TestContext Framework
  ** uses
    *** | class level
      **** -- alternative to -- `@ContextConfiguration` + `@WebAppConfiguration`
  ** 's configuration options vs `@ContextConfiguration` 's configuration options
    *** you can declare component classes by using the
