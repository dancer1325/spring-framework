...
* Spring Framework does NOT contribute Observations -- for -- xref:core/beans/context-introduction.adoc#context-functionality-events-annotation[`@EventListener` calls]
  ** Reason: 🧠NOT have the right semantics -- for -- such instrumentation 🧠
* event publication & event processing
  ** by default, are done
    *** synchronously
    *** | SAME thread
      **** -> 👀ThreadLocals & logging context | task execution == ThreadLocals & logging context | event publisher 👀
      **** if the application globally configures a custom `ApplicationEventMulticaster` / strategy schedules event processing | different threads -> previous NO longer true
* ALL `@EventListener` methods are processed | DIFFERENT thread / -- OUTSIDE the -- main event publication thread
  ** https://micrometer.io/docs/contextPropagation[Micrometer Context Propagation library] can
    *** help propagate such values
    *** -- better correlate the -- processing of the events
* application can configure the chosen `TaskExecutor` -- to use -- a `ContextPropagatingTaskDecorator`
  ** /
    *** decorates tasks
    *** propagates context
  ** requirements
    *** `io.micrometer:context-propagation` library | classpath
