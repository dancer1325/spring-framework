...
* Spring Framework -- provides --
  ** abstractions -- for the --
    *** asynchronous execution of tasks -- via -- `TaskExecutor`
    *** asynchronous scheduling of tasks -- via -- `TaskScheduler`
  ** implementations of interfaces /
    *** -- support --
      **** thread pools | application server environment
      **** delegation to CommonJ | application server environment
    *** it's use / behind the interfaces -> abstracts away the differences Java SE environments vs Jakarta EE environments
  ** integration classes / -- support scheduling with the --
...
* Executors
  ** == JDK name -- for the concept of -- thread pools
  ** Reason of the name: 🧠 NO guarantee / underlying implementation is a pool 🧠
  ** types
    *** 1!-threaded
    *** synchronous
* Spring's
  ** abstraction -- hides -- implementation details between Java SE environments vs Jakarta EE environments
  ** `TaskExecutor` interface
    *** == `java.util.concurrent.Executor` interface
    *** Reason for existence: 🧠
      **** abstract away the need for Java 5 | using thread pools
      **** give other Spring components an abstraction | thread pooling / was needed 🧠
    *** `execute(Runnable task)`
      **** 1! method of the interface / task as input for execution -- based on -- thread pool
        ***** semantics
        ***** configuration
    *** uses
      **** pool threads
        ***** _Example:_ by `ApplicationEventMulticaster`, JMS's `AbstractMessageListenerContainer`, Quartz integration
      **** thread pooling behavior -- for your -- beans

[[scheduling-task-executor-types]]
=== `TaskExecutor` Types

* Spring's pre-built implementations of `TaskExecutor`
  ** `SyncTaskExecutor`
    *** does NOT run invocations asynchronously
      **** each invocation | calling thread
    *** uses
      **** situations / multi-threading NOT necessary
        ***** _Example:_ simple test cases
  ** `SimpleAsyncTaskExecutor`
    *** does NOT reuse any threads
      **** == NEW thread / each invocation
      **** 👀concurrency limit == maximum # of threads 👀
        ***** if invocations > limit -> wait UNTIL a slot is freed up
        ***** != true pooling
    *** Spring v6.1+, provides
      **** graceful shutdown capabilities
    *** "virtualThreads" option / -- aligned with -- JDK 21's Virtual Threads
  ** `ConcurrentTaskExecutor`
    *** == adapter for a `java.util.concurrent.Executor` instance
    *** uses
      **** RARELY
      **** if `ThreadPoolTaskExecutor` is NOT flexible enough
  ** `ThreadPoolTaskExecutor`
    *** uses
      **** MOSTLY
    *** exposes bean properties -- for --
      **** configuring a `java.util.concurrent.ThreadPoolExecutor`
      **** wrapping it | `TaskExecutor`
    *** Spring v6.1+, provides the capabilities
      **** pause/resume
      **** graceful shutdown
        ***** -- through -- Spring's lifecycle management
  ** `DefaultManagedTaskExecutor`
    *** -- uses a -- JNDI-obtained `ManagedExecutorService` | JSR-236
  compatible runtime environment (_Example:_ Jakarta EE application server)
      **** -> -- replace a -- CommonJ WorkManager
...
* allows
  ** | method -> invocation of that method occurs asynchronously
    *** ==
      **** caller returns immediately | invocation
      **** execution of the method | task / -- has been submitted to a -- Spring `TaskExecutor`
    *** _Example:_
    ...
    *** 👀vs methods / annotated with `@Scheduled` annotation 👀
      **** methods / annotated with `@Async` -- can --
        ***** accept -- arguments
          ****** Reason: 🧠the invokers are "`normal`" callers | runtime, != scheduled task / -- managed by the -- container 🧠
          ****** _Example:_
        ...
        ***** return -- a value
          ****** requirements
            ******* `Future`-typed return value
              ******** caller -- can perform -- other tasks | prior to calling `Future.get()`
              ******** BUT, Spring v4.2+, could be also
...
* `@Async` + lifecycle callbacks (_Example:_ `@PostConstruct`) NOT possible
  ** 👀if you want too asynchronously initialize Spring beans -> use a separate
initializing Spring bean / invokes `@Async` annotated method | target 👀
    *** _Example:_
...
* 👀NO == | XML 👀
  ** alternative
    *** manually set up Spring's `AsyncExecutionInterceptor` + Spring AOP / exist custom pointcut
