...
* goal
    ** hints & recommendations -- for developing -- Spring | Kotlin

[[final-by-default]]
== Final by Default

* By default, https://discuss.kotlinlang.org/t/classes-final-by-default/166[ALL classes & member functions | Kotlin are `final`]
* `open`
    ** := modifier
    ** allows
        *** others -- can inherit from -- this class or member functions
    ** uses
        *** | class
        *** | member functions
    ** -- opposite of -- Java's `final`
    ** 👁️add | ALL class & member function of Spring beans / proxied by CGLIB 👁️
        *** Reason: 🧠Spring beans are normally proxied by CGLIB, and else, Spring Boot application could prevent from starting  🧠
        *** 👁️ it goes against the Kotlin principle of keeping code concise and predictable 👁️
        *** alternative
            **** use `@Configuration(proxyBeanMethods = false)`
* {kotlin-docs}/compiler-plugins.html#kotlin-spring-compiler-plugin[`kotlin-spring`]
    ** == plugin /
        *** -- preconfigured version of the -- `kotlin-allopen` plugin
        *** 👁️AUTOMATICALLY `open` classes & their member functions / annotated or meta-annotated with one of 👁️
            **** `@Component`
                ***** -> since it affects ALSO meta-annotations
                    ****** `@Configuration`
                    ****** `@Controller`
                    ****** `@RestController`
                    ****** `@Service`
            ...
            **** `@Transactional`
            **** `@Cacheable`
    ** used by default, by https://start.spring.io/#!language=kotlin&type=gradle-project[start.spring.io]
* proxies + automatic generation of final methods by the Kotlin compiler extra care
    ** Kotlin class / properties -- will generate -- related `final` getters & setters
    ** if you want to proxy related methods -> `@Component` instead of `@Bean`
        *** Reason: 🧠have those methods -- opened by the -- `kotlin-spring` plugin 🧠
    ** use case
        *** `@Scope` & `@RequestScope`
* Kotlin code samples | Spring Framework documentation
    ** do NOT specify `open` | classes & their member functions
    ** using the `kotlin-allopen` plugin
...
* ways to inject configuration properties
    ** | Java
        *** -- via -- annotations
            **** pass:q[`@Value("${property}")`)]
               ***** NOT valid | Kotlin, because 🧠 `$` is a reserved character -- for -- {kotlin-docs}/idioms.html#string-interpolation[string interpolation] 🧠
            **** if you use Spring Boot -> use {spring-boot-docs}/boot-features-external-config.html#boot-features-external-config-typesafe-configuration-properties[`@ConfigurationProperties`]
    ** | Kotlin
        *** -- via -- annotations
            ****  pass:q[`@Value("\${property}")`]
                ***** escaping  `$` == `\$`
            **** if you use Spring Boot -> use {spring-boot-docs}/boot-features-external-config.html#boot-features-external-config-typesafe-configuration-properties[`@ConfigurationProperties`]
* configuring bean `PropertySourcesPlaceholderConfigurer` -> you can customize
    ** property placeholder prefix
    ...
    ** existing code ( _Example:_ Spring Boot actuators or `@LocalServerPort`)
