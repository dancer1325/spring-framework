...
* `org.springframework.beans.factory` package
  ** provides about beans, basic functionality (ALSO programmatically) for
    *** managing
    *** manipulating
  ** Check xref:core/beans/basics.adoc[chapter introduction]
* `org.springframework.context` package
  ** adds the {spring-framework-api}/context/ApplicationContext.html[`ApplicationContext`] / extends in a way framework-oriented style
    *** `BeanFactory` interface
    *** other interfaces
  ** uses
    *** declarative fashion
      **** _Example:_ via `ContextLoader` -- to automatically instantiate an -- `ApplicationContext` / part of the normal startup process of a Jakarta EE web application
    *** enhance `BeanFactory` functionality / more framework-oriented style
      **** `MessageSource`
        ***** -- access to messages in -- i18n-style
      **** `ResourceLoader`
        ***** -- access to -- resources (_Example:_ URLs and files)
      **** `ApplicationEventPublisher`
        ***** -- publish events == beans / implement `ApplicationListener`
      **** `HierarchicalBeanFactory`
        ***** -- load -- several (hierarchical) contexts / each one can be focused | particular layer (_Example:_ web layer)
...
* `HierarchicalMessageSource`
  ** resolve messages hierarchically
  ** some implementations of it
    *** `ResourceBundleMessageSource`
    *** `ReloadableResourceBundleMessageSource`
    *** `StaticMessageSource`
* `MessageSource` + `HierarchicalMessageSource` == base of Spring message resolution
* `ApplicationContext` extends `MessageSource`
  ** when an `ApplicationContext` is loaded -> 👁️automatically searches for a `MessageSource` bean | context / 👁
    *** bean name -- must be -- `messageSource`
    *** if the bean is NOT found -> `ApplicationContext` attempts to find a parent / contains a bean with name `messageSource` / it's used as the `MessageSource`
    *** if NO bean is found for messages -> empty `DelegatingMessageSource` is instantiated
* _Example1:_ `ResourceBundleMessageSource` -- Check 'examples/third' --
    ...
    <beans>
		<bean id="messageSource"
			...
			<property name="basenames">
                <!-- next "basenames" must be defined in the classpath -->
				<list>
					<value>format</value>
...
----
    ...
    # let's be content of the resource bundle files
...
* _Example2:_ run the `MessageSource` functionality
...
// TODO: Check and pass to the example!!
