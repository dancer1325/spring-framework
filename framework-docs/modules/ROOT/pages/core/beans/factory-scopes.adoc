...
* Spring IoC container manages
  ** instantiation of your objects (beans)
  ** wiring up of collaborators (or dependencies)
    *** 👁️ alternatives to inject a bean 👁️
      **** inject an AOP proxy
        ***** Example:
          ****** inject an HTTP request-scoped bean -- into -- another bean of a longer-lived scope vs
          ****** inject an AOP proxy /
            ******* public interface exposed == scoped object exposed
            ******* -- from the relevant scope, retrieve -- real target object -- _Example:_ HTTP request --
            ******* -- delegate method calls -- onto the real object
      **** if beans are `singleton` scoped -> use `<aop:scoped-proxy/>`
        ***** == intermediate proxy / is serializable
          ****** -- able to re-obtain -- target singleton bean | deserialization
      **** if beans are `prototype` scoped -> use `<aop:scoped-proxy/>`
        ***** -> ALL call | shared proxy -- leads to create -- new target instance | call is forwarded
      **** declare the injection point (_Example:_ constructor, setter, ...) -- as -- `ObjectFactory<MyTargetBean>`
        ***** -> `getObject()` retrieves the current instance | demand every time it is needed / NO holding on the instance or storing separately
        ***** extended variant, declaring
          ****** `getIfAvailable`
          ****** `getIfUnique`
      **** JSR-330 variant == `Provider<MyTargetBean>` + `get()` call | every retrieval attempt
