...
* := actual objects / make up your application
  ** how to define them?
    *** service layer objects, persistence layer objects -- are defined as -- repositories or data access objects (DAOs)
    *** presentation objects -- are defined as -- Web controllers
    *** infrastructure objects -- are defined as -- JPA `EntityManagerFactory`, JMS queues, and so forth.
    *** fine-grained 👁️ domain objects are NOT defined in the container 👁️
      **** Reason: 🧠 domain objects are responsibility of repositories and business logic 🧠
  ** -- via configuring metadata / supplied to the container -> are created
