...
* [.small]#xref:web/webflux/ann-rest-exceptions.adoc[== in the Reactive stack]#
* common requirement for REST services / errors
  ** include details in the errorResponseBody
  ** Spring Framework supports the "Problem Details for HTTP APIs" specification, {rfc-site}/rfc9457[RFC 9457]
  ** main abstractions
    *** `ProblemDetail`
      **** -- representation for -- an RFC 9457 problem detail
      **** == simple container for
        ***** standard fields | spec
        ***** non-standard fields | spec
    *** `ErrorResponse`
      **** := interface / expose HTTP error response details
        ***** HTTP status
        ***** response headers
        ***** body in the format of RFC 9457
      **** allows
        ***** exceptions -- can map to -- an HTTP response
      **** -- implemented by -- ALL Spring MVC exceptions
    *** `ErrorResponseException`
      **** == basic `ErrorResponse` implementation
    *** `ResponseEntityExceptionHandler`
      **** uses
        ***** annotated with xref:web/webmvc/mvc-controller/ann-advice.adoc[@ControllerAdvice] /
          ****** handles ALL Spring MVC exceptions & ANY `ErrorResponseException`
...
* [.small]#xref:web/webflux/ann-rest-exceptions.adoc#webflux-ann-rest-exceptions-render[== in the Reactive stack]#
