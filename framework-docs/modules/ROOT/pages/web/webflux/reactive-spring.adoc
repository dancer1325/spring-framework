...
* `ProtobufEncoder` and `ProtobufDecoder`
	** support decoding and encoding
		*** "application/x-protobuf",
		*** "application/octet-stream"
		*** "application/vnd.google.protobuf" content -- for -- `com.google.protobuf.Message` types
		*** if content is received/sent + "delimited" parameter + along the content type (_Example:_ "application/x-protobuf;delimited=true") -> stream of values
	** requirements
		*** "com.google.protobuf:protobuf-java" library v3.29+
* `ProtobufJsonDecoder` and `ProtobufJsonEncoder`
	** support reading and writing JSON documents <- to and from -> Protobuf messages
		*** ⚠️ NOT support reading stream of messages ⚠️
	** requirements
		*** "com.google.protobuf:protobuf-java-util" dependency
	** Check {spring-framework-api}/http/codec/protobuf/ProtobufJsonDecoder.html[javadoc of `ProtobufJsonDecoder`]
